...
function wireLikes() {
  ...
  alreadyLiked =
    localStorage.getItem(`liked_lesson_${lessonId}_${currentUser.id}`) === "1";
  likeSwitch.checked = alreadyLiked;
  likeSwitch.disabled = alreadyLiked;
  ...
  likeSwitch.addEventListener("change", async () => {
    ...
    try {
      ...
      if (res.ok) {
        alreadyLiked = true;
        localStorage.setItem(`liked_lesson_${lessonId}_${currentUser.id}`, "1");
        await loadLikes();
      }
    } catch (err) {
      console.log(err);
