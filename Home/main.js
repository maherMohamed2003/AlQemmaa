  ...
  return `
    <div class="qa-item" data-question-id="${q.id}">
      ...
      <div class="qa-bubble">
        ...
        <div class="qa-edit-box" hidden>
          <textarea class="qa-edit-input">${escapeHtml(q.content || "")}</textarea>
          <div class="qa-edit-actions">
            <button type="button" class="btn-sm qa-edit-save">حفظ</button>
            <button type="button" class="btn-sm qa-edit-cancel">إلغاء</button>
          </div>
        </div>
        ${
          canManage
            ? `<div class="qa-manage-actions">
                <button type="button" class="qa-edit" data-id="${q.id}" aria-label="تعديل السؤال">
...
// onDelete(id) و onEdit(id, newContent) بيتبعتوا من بره
function openCommentsModal(questions, onDelete, onEdit) {
  if (!questions || questions.length === 0) {
    ...
    commentsModalList.querySelectorAll(".qa-edit").forEach((btn) => {
      btn.addEventListener("click", () => {
        const item = btn.closest(".qa-item");
        item.querySelector(".qa-view").hidden = true;
        item.querySelector(".qa-edit-box").hidden = false;
      });
    });

    commentsModalList.querySelectorAll(".qa-edit-cancel").forEach((btn) => {
      btn.addEventListener("click", () => {
        const item = btn.closest(".qa-item");
        item.querySelector(".qa-edit-box").hidden = true;
        item.querySelector(".qa-view").hidden = false;
      });
    });

    commentsModalList.querySelectorAll(".qa-edit-save").forEach((btn) => {
      btn.addEventListener("click", () => {
        const item = btn.closest(".qa-item");
        const id = item.dataset.questionId;
        const newContent = item.querySelector(".qa-edit-input").value.trim();
...
function attachCardHandlers() {
  document.querySelectorAll(".post-card").forEach((card) => {
    ...
    function isMyLike(l) {
      const uid =
        l.userId ?? l.UserId ?? l.studentId ?? l.StudentId ?? l.userID;
    ...
    async function handleDeleteQuestion(id) {
      try {
        ...
        if (res.status === 401 || res.status === 403) {
          alert("مفيش صلاحية لحذف هذا السؤال");
          return;
        }
      } catch (err) {}
      await loadQuestions();
      if (questions.length) {
        openCommentsModal(questions, handleDeleteQuestion, handleEditQuestion);
      } else {
        closeCommentsModal();
      }
    }

    async function handleEditQuestion(id, newContent) {
      try {
        const res = await fetch(`${QUESTION_API}/EditQuestion`, {
          method: "PUT",
          headers: {
        ...
        if (res.status === 401 || res.status === 403) {
          alert("مفيش صلاحية لتعديل هذا السؤال");
          return;
        }
    ...
    // نفس الزرار بيعمل لايك أو إلغاء لايك — السيرفر هو اللي بيقرر لأن AddNewLikeToPost
