...
let lessons = [];
let rawGroups = null; // آخر نتيجة خام لجروبات السنين (وضع المعلم/المبرمج) قبل تطبيق البحث
let searchQuery = "";
...
// بيرجع true لو الدرس مطابق لنص البحث الحالي (بالعنوان أو الوصف)
...
// بيطبّق البحث على آخر بيانات محمّلة (سواء مجموعة بالسنين أو قائمة عادية) ويعرضها
function RenderFiltered() {
  if (rawGroups) {
    const filteredGroups = rawGroups.map((g) => ({
      ...g,
      lessons: (g.lessons || []).filter(matchesSearch),
    }));
    RenderGroupedLessons(filteredGroups);
  } else {
    RenderLessons(lessons.filter(matchesSearch));
  }
}
...
function RenderLessons(list) {
  ...
  if (!list || list.length == 0) {
    lessonsContainer.innerHTML = `<div class="empty-state">${
      searchQuery.trim()
