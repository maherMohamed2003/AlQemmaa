...
// ---------- form submit ----------
const API_URL = `${(window.APP_CONFIG && window.APP_CONFIG.API_BASE) || "https://abdomahne.runasp.net/api"}/User/Register`;
...
function setLoading(isLoading) {
  ...
  btnText.textContent = isLoading ? "جارِ إنشاء الحساب..." : "إنشاء الحساب";
}

function validate() {
  let ok = true;
  [nameField, yearField, phoneField, emailField, passwordField].forEach((f) =>
    f.classList.remove("invalid"),
  );
  [nameError, yearError, phoneError, emailError, passwordError].forEach(
    (e) => (e.textContent = ""),
  );
  ...
  if (nameVal.length < 3) {
    ...
    nameError.textContent = "من فضلك أدخل الاسم بالكامل";
    ok = false;
  }
  if (!yearVal) {
    yearField.classList.add("invalid");
    yearError.textContent = "من فضلك اختر الصف الدراسي";
    ok = false;
  }
  const phonePattern = /^01[0125][0-9]{8}$/;
  if (!phonePattern.test(phoneVal)) {
    phoneField.classList.add("invalid");
    phoneError.textContent = "أدخل رقم هاتف مصري صحيح";
    ok = false;
  }
  ...
  if (!pwVal || pwVal.length < 6) {
    ...
    passwordError.textContent = "كلمة المرور يجب ألا تقل عن 6 أحرف";
    ok = false;
  }
  return ok;
}
